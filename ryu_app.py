...
class SDNFirewallLoadBalancer(app_manager.RyuApp):
    ...
    def packet_in(self, ev):
        ...
        # ============================
        # HANDLE TRAFFIC VIP (Klien -> VIP -> Server)
        # ============================
        if ip_pkt and ip_pkt.dst == self.VIP:
            ...
            if self.cek_ddos(src_ip):
                self.logger.warning("Request dari %s ditolak oleh firewall", src_ip)
                return
            ...
            if tcp_pkt and tcp_pkt.dst_port == 80:
                ...
                # 2. TRAFFIC ENGINEERING: Trafik SSH (Port 22) HANYA ke Server 1
                server_ip = "10.0.0.2"
                self.logger.info("[TRAFFIC STEERING] Trafik SSH dipaksa ke %s", server_ip)
            
            elif icmp_pkt:
                # 3. TRAFFIC ENGINEERING: Trafik Ping dipaksa HANYA ke Server 2
                server_ip = "10.0.0.3"
            ...
            server_mac = self.SERVER_POOL[server_ip]["mac"]
            server_port = self.SERVER_POOL[server_ip]["port"]

            # NAT: Ubah MAC dst dan IP dst menjadi milik Server asli
            actions = [
        ...
        # ============================
        # HANDLE REVERSE TRAFFIC (Server -> Klien)
        # ============================
        if ip_pkt and ip_pkt.src in self.SERVER_POOL.keys():
            # Jika paket berasal dari server, kita samarkan seolah dari VIP
            actions = [
                parser.OFPActionSetField(eth_src=self.LB_MAC),
                parser.OFPActionSetField(ipv4_src=self.VIP),
                parser.OFPActionOutput(1) # Asumsi h1 (Klien) selalu di port 1
            ]
        ...
        # ============================
        # FLOODING UNTUK ARP BIASA (PENTING AGAR PING BISA JALAN)
        # ============================
